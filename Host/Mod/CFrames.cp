MODULE HostCFrames;
	...
	(* TODO:
		- Mouse events are not recieved when the widget is disabled
	...
	*)
	...
	(* 
		...
		BlackBox wants to filter mouse and keyboard events. To do this special handlers for these events are installed 
		(done in NewInfo). These handlers basically stops the events and resends the event only if the event comes
		back through the BlackBox framework. 
	...
	BEGIN			
		...
		IF ~i.allowMouse THEN		
			...
			IF (i.frame.rider(HostPorts.Rider).port.da # NIL) & (event.type = LinGdk.GDK_BUTTON_PRESS) THEN
				...
				mouseDelayer.delayedEvent.send_event := LinLibc.TRUE; 
				mouseDelayer.forwardEvent := event; 
				mouseDelayer.forwardEvent.send_event := LinLibc.TRUE;
...
*)
	...
	PROCEDURE (c: ComboBox) Update;
		VAR s: Dialog.String; ss: ARRAY LEN(Dialog.String) OF SHORTCHAR;
	BEGIN
		IF c.combo = NIL THEN RETURN END;
		IF c.combo(LinGtk.GtkCombo).entry = NIL THEN RETURN END;
		c.isUpdate := TRUE;
		IF ~c.disabled THEN
			c.Get(c, s); Dialog.MapString(s, s);
			ss := SHORT(s);
			LinGtk.gtk_entry_set_text(c.combo(LinGtk.GtkCombo).entry, ss);
			IF ~c.readOnly THEN
				LinGtk.gtk_widget_set_sensitive(c.combo, LinLibc.TRUE)
			ELSE
				LinGtk.gtk_widget_set_sensitive(c.combo, LinLibc.FALSE)
			END
		ELSE
			LinGtk.gtk_entry_set_text(c.combo(LinGtk.GtkCombo).entry, "");
			LinGtk.gtk_widget_set_sensitive(c.combo, LinLibc.FALSE)
		END;
		c.isUpdate := FALSE;
		LinGtk.gtk_widget_show_all(c.combo)
(*	
		IF ~c.disabled THEN
			c.Get(c, value);
			IF c.undef THEN 
...
*)
	...
	BEGIN
		IF c.combo = NIL THEN
			...
			res := LinGtk.gtk_signal_connect(c.combo(LinGtk.GtkCombo).entry, "button_press_event", 
													SYSTEM.VAL(LinGtk.GtkSignalFunc, SYSTEM.ADR(MouseHandler)), 
													SYSTEM.VAL(INTEGER, c.i));
			res := LinGtk.gtk_signal_connect_after(c.combo(LinGtk.GtkCombo).entry, "button-release-event", 
													SYSTEM.VAL(LinGtk.GtkSignalFunc, SYSTEM.ADR(MouseRelease)), 
													SYSTEM.VAL(INTEGER, c.i));
			res := LinGtk.gtk_signal_connect(c.combo(LinGtk.GtkCombo).entry, "key-press-event", 
													SYSTEM.VAL(LinGtk.GtkSignalFunc, SYSTEM.ADR(KeyHandler)), 
													SYSTEM.VAL(INTEGER, c.i));
	(*
			res := LinGtk.gtk_signal_connect(c.combo(LinGtk.GtkCombo).button, "button_press_event", 
													SYSTEM.VAL(LinGtk.GtkSignalFunc, SYSTEM.ADR(MouseHandler)), 
													SYSTEM.VAL(INTEGER, c.i));
			res := LinGtk.gtk_signal_connect_after(c.combo(LinGtk.GtkCombo).button, "button-release-event", 
													SYSTEM.VAL(LinGtk.GtkSignalFunc, SYSTEM.ADR(MouseRelease)), 
													SYSTEM.VAL(INTEGER, c.i));
			res := LinGtk.gtk_signal_connect(c.combo(LinGtk.GtkCombo).button, "key-press-event", 
													SYSTEM.VAL(LinGtk.GtkSignalFunc, SYSTEM.ADR(KeyHandler)), 
													SYSTEM.VAL(INTEGER, c.i));
	*)		
	...
	BEGIN
		IF c.combo # NIL THEN
			LinGtk.gtk_widget_unref(c.combo);
...
*)
	...
	PROCEDURE (c: ScrollBar) Update;
		VAR  size, sect, pos, q, m: INTEGER; adj: LinGtk.GtkAdjustment; trans: REAL;
	BEGIN				
		IF ~c.disabled THEN
			c.Get(c, size, sect, pos);
...
(*		Adapt(f, f.i)*)
	...
	BEGIN
		...
		IF ~f.disabled THEN
			...
			END
		ELSE
			LinGtk.gtk_entry_set_text(f.spin, "");
			LinGtk.gtk_widget_set_sensitive(f.spin, LinLibc.FALSE)
		END;
		f.isUpdate := FALSE
		;UpdateEventMask (f.i)
