MODULE HostPackedFiles;
	...
	(* bj	19.10.01	Linux version. Chaged Init to use HostFiles.appName directly *)
	(* bj	26.03.01	support for network files added *)
	(* ww	05.02.01	in ReadBytes buffer changed from OUT to VAR parameter *)
	(* bj	03.11.00	added assertion in Register *)
	(* bj	31.10.00	added ask parameter to File.Register, Directory.New and Directory.Rename *)
	(* bj	23.10.00	Removed imports of Stores, Strings and Dialog (Dialog still needed for debugging) *)
	
	(* This module depends on the implementation of DevPacker. *)

	IMPORT SYSTEM, LinLibc, Files, HostFiles(*, Dialog*);
...
(*	BEGIN
	...
	BEGIN
		...
		ELSIF (*(path[0] = '/') OR *) (path[0] = '\')  THEN (* network path *)
			...
			ELSE
				...
				RETURN NIL
			END
		ELSIF (path[1] = ':') OR (path[0] = '/') THEN (* absolute path *)
			GetNextSubLoc(path, 1, sp, end);
		ELSE
	...
	BEGIN
		...
		r.SetPos(exefile.Length() - 8);
		ReadInt(r, int); 
		IF int = magicnum THEN
			ReadInt(r, tableadr);
	...
	PROCEDURE (d: Directory) FileList(floc: Files.Locator): Files.FileInfo;
		VAR fi, nfi, tfi, last: Files.FileInfo; loc: Locator; l: FileList; type: Files.Type; hloc: HostFiles.Locator;
			diff: INTEGER;
	BEGIN
		...
		IF loc # NIL THEN
			l := loc.files; nfi := NIL; last := NIL; tfi := NIL;
			(* Both fi and l are alphabetically sorted. And the returned list also has to be alphabetically sorted. *)
			WHILE (l # NIL) & (fi # NIL) DO
				diff := Diff(l.name, fi.name, hloc.caseSens);
				IF diff >= 0 THEN
					tfi := fi;
					fi := fi.next; IF diff = 0 THEN l := l.next END
				ELSE
					GetType(l.name, type);
	...
	PROCEDURE (d: Directory) LocList (floc: Files.Locator): Files.LocInfo;
		VAR li, nli, tli, last: Files.LocInfo; loc: Locator; hloc: HostFiles.Locator; diff: INTEGER;
	BEGIN
		li := orgdir.LocList(floc);  hloc := floc(HostFiles.Locator); nli := NIL;
		loc := GetLoc(hloc.path$, FALSE);
		IF loc # NIL THEN loc := loc.sublocs END;
		(* Both loc and li are alphabetically ordered. *)
		WHILE (loc # NIL) & (li # NIL) DO
			diff := Diff(loc.name, li.name, hloc.caseSens);
			IF diff >= 0 THEN
				IF nli = NIL THEN nli := li ELSE last.next := li END; last := li;
				li := li.next; IF diff = 0 THEN loc := loc.next END
			ELSE
				NEW(tli); tli.name := loc.name; tli.attr := {Files.readOnly};
				IF nli = NIL THEN nli := tli ELSE last.next := tli END; last := tli;
				loc := loc.next
			END
		END;
		IF loc = NIL THEN
	...
	PROCEDURE Init;
		VAR loc: Files.Locator;  appName: Files.Name;
	BEGIN
		loc := Files.dir.This(""); 
	(*	Files.dir.GetFileName(HostFiles.appName$, "", appName)*)
		(*Files.dir.GetFileName(HostFiles.appName$, "EXE", appName) *)
		appName := HostFiles.appName$;
	...
	PROCEDURE PrintLoc(loc: Locator; depth: INTEGER);
		VAR l: FileList; str: ARRAY 256 OF CHAR; i: INTEGER;
	BEGIN
		IF loc # NIL THEN
			FOR i := 0 TO depth DO str[i] := " " END; str[i] := 0X;
			DebugPrint( str + "Locator: " + loc.name + " - - - - - - - - - - - - - ");
			l := loc.files; 
			WHILE l # NIL DO DebugPrint(l.name); l := l.next END; 
			PrintLoc(loc.sublocs, depth + 2);
			PrintLoc(loc.next, depth)
		END
	END PrintLoc;
