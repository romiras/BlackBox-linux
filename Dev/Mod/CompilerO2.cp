MODULE DevCompilerO2;
(* rc 2.7.92 / mf 8.1.93 / eo 4.2.93 / mf 4.2.93 / cp 23 Sep 93 / bh 19.6.94 *)

	(* bh	09.09.99	support of DevSelectors *)
	(* dg	11.04.99	CompileText: VAR => OUT *)
	(* dg	03.03.99	changed Kernel.ThisMod => Kernel.ThisLoadedMod *)
	(* dg	21.01.99	module name now also appears in log if module starts with comment *)
	(* Component Pascal version, bh, 09 Sep 1999 *)
	...
	CONST
		...
		defopt = {checks, assert, obj, ref, allref, srcpos, oberon};
	...
	BEGIN
		...
		WHILE (s.start < end) & (s.type = TextMappers.string) & (s.len < LEN(name)) DO
			...
			WHILE (s.type = TextMappers.char) &
				((s.char = "-") OR (s.char = "+") OR
				(s.char = "!") OR (s.char = "*") OR (s.char = "?") OR (s.char = "^") OR (s.char = "(")) DO
				IF s.char = "(" THEN
					WHILE (s.type # TextMappers.eot) & ((s.type # TextMappers.char) OR (s.char # ")")) DO s.Scan END
				END;
				s.Scan
			END
		END;
