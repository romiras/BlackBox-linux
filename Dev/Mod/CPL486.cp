MODULE DevCPL486;  (* bh 16.11.93 / 7.9.94 *)
...
*)
	...
	BEGIN
		...
		WHILE a <= e DO
			DevCPE.PutByte(a, DevCPE.pc); DevCPE.PutByte(a + 1, DevCPE.pc DIV 256); DevCPE.PutByte(a + 2, DevCPE.pc DIV 65536);
			INC(a, 4)
		END;
		a1.mode := 0; a2.mode := 0
	END CaseEntry;
		...
		IF to.mode = Reg THEN
			IF from.mode = Con THEN
				IF to.reg = AX THEN
					...
					IF (a1.mode = Con) & (from.offset = a1.offset) & (from.obj = a1.obj) & (from.form = a1.form) THEN RETURN END;
			...
			ELSIF (to.reg = AX) & (from.mode = Abs) & (from.scale = 0) THEN
				...
				IF (a1.mode = Abs) & (from.offset = a1.offset) & (from.obj = a1.obj) & (from.form = a1.form)
					OR (a2.mode = Abs) & (from.offset = a2.offset) & (from.obj = a2.obj) & (from.form = a2.form) THEN RETURN END;
				...
				IF to.reg = AX THEN
					IF (from.mode = Ind) & (from.scale = 0) & ((from.reg = BP) OR (from.reg = BX)) THEN
						...
						IF (a1.mode = Ind) & (from.offset = a1.offset) & (from.reg = a1.reg) & (from.form = a1.form)
							OR (a2.mode = Ind) & (from.offset = a2.offset) & (from.reg = a2.reg) & (from.form = a2.form) THEN RETURN END;
	...
	BEGIN
		...
		ELSIF src.mode = Con THEN
			...
			IF (src.obj = NIL) & (val # 0) & ~ovflchk THEN
				...
				IF ABS(val) <= 3 THEN
					WHILE f9 > 0 DO DevCPE.GenByte(8DH); DevCPE.GenByte(dst.reg * 8 + 4); DevCPE.GenByte(0C0H + dst.reg * 9); DEC(f9) END;
					WHILE f5 > 0 DO DevCPE.GenByte(8DH); DevCPE.GenByte(dst.reg * 8 + 4); DevCPE.GenByte(80H + dst.reg * 9); DEC(f5) END;
					IF ABS(val) = 3 THEN DevCPE.GenByte(8DH); DevCPE.GenByte(dst.reg * 8 + 4); DevCPE.GenByte(40H + dst.reg * 9) END;
...
*)	
	...
	PROCEDURE GenStringComp* (wsize, dsize: INTEGER);	(* wsize # dsize -> convert; size = 0: opsize = 1, incsize = 2; *)
		VAR loop, end: Label;
	BEGIN
