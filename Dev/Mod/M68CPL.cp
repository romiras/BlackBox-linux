...
(*	PROCEDURE FJump*(condition: INTEGER; VAR L: Label);
	...
	BEGIN
		IF item.mode IN {dreg, areg, freg, postinc, predec, regx} THEN
			...
			END;
			IF (item.mode = dreg) & (item.typ # NIL) & (item.typ.size = 8) THEN	(* bh *)
	...
	VAR
		...
		sBd, dBd: INTEGER;
	BEGIN
		IF (source.mode = imm) & (source.typ.form IN {Bool..Int32, Set, NilTyp, Pointer}) & (source.bd = 0) THEN
			Clr(dest);
			RETURN
		END;
...
(*					FLoad(dest);*)
	...
	VAR
		mode, reg, extWord, format, size, len: SHORTINT;
		bd: INTEGER;
	BEGIN
		IF dest.mode = areg THEN 
			IF source.typ.size = 2 THEN len := 3 ELSE len := 7 END
		ELSE
			Load(dest); len := LengthCode(source.typ.size)
		END;
		Encode(source, mode, reg, extWord, format, bd, 0);
		DevCPE.GenShort(0B000H + SYSTEM.LSH(dest.reg MOD 8, 9) + SYSTEM.LSH(len, 6)
		 + SYSTEM.LSH(mode, 3) + reg);
		GenExtension(format, extWord, bd, source);
	END Cmp;
