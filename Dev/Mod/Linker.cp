...
*)
	...
	BEGIN
			...
			ELSE
				...
				IF r.file # NIL THEN
					IF r.typ = -1 THEN	(* typelib *)
						r.typ := 0; r.size := r.file.Length(); r.pos := 0
					ELSE
						...
						ELSE
							...
							ELSE
								...
								IF (x = 0) & (n = 20H) THEN	(* resource file *)
									...
									Read4(r.size); Read4(n); Read2(i); Read2(j);
									IF (i = 0FFFFH) & (j >= 4) & ((j <= 11) OR (j = 16)) THEN
										r.typ := j; r.pos := n + 32;
									ELSE
		...
		WHILE res # NIL DO
			r := res; res := res.next;
			IF (resList = NIL) OR (r.typ < resList.typ) OR (r.typ = resList.typ) & ((r.id < resList.id) OR (r.id = resList.id) & (r.lid < resList.lid)) THEN
		...
		WHILE r # NIL DO
			...
			END
		END;
		IF numId[0] > 0 THEN INC(n, 16) END;
		RsrcSize := resHSize + n;
	...
	BEGIN
		IF m # NIL THEN
			...
			ELSE
				...
				SearchObj(m, terminator, mProc, terminatorFP, -1, x);
				IF x = -1 THEN
...
*)	
	...
	PROCEDURE WriteCode;
		VAR mod, m: Module; i, x, a, fp, opt: INTEGER; ch: SHORTCHAR; exp: Export; name: Name;
	BEGIN
		IF isStatic THEN
		...
		WHILE mod # NIL DO impg := mod; impd := mod;
			IF ~mod.dll THEN
				...
				IF x # 0 THEN
					IF (mod # kernel) & (kernel # NIL) THEN
						SearchObj(kernel, newRec, mProc, NewRecFP, 0, a); Fixup0(x, a)
					ELSE
						W.WriteSString("no kernel"); W.WriteLn;
				...
				IF x # 0 THEN
					IF (mod # kernel) & (kernel # NIL) THEN
						SearchObj(kernel, newArr, mProc, NewArrFP, 0, a); Fixup0(x, a)
					ELSE
						W.WriteSString("no kernel"); W.WriteLn;
						Log.text.Append(Log.buf); error := TRUE; RETURN
					END
	...
	PROCEDURE WriteConst;
		VAR mod, m, last: Module; x: INTEGER;
	BEGIN
	...
	PROCEDURE WriteResource;
		VAR res, r, s: Resource; i, t, a, x, n: INTEGER;
	BEGIN
		IF numId[0] > 0 THEN WriteResDir(1, numType - 1)
		...
		END;
		ASSERT(a = resHSize);
		IF numId[0] > 0 THEN INC(a, 16) END;	(* TYPELIB string *)
		r := resList;
		WHILE r # NIL DO
		...
		ASSERT(a = RsrcSize);
		IF numId[0] > 0 THEN
			Write2(7); Write2(ORD("T")); Write2(ORD("Y")); Write2(ORD("P"));
			Write2(ORD("E")); Write2(ORD("L")); Write2(ORD("I")); Write2(ORD("B"))
		END;
		r := resList;
		WHILE r # NIL DO
