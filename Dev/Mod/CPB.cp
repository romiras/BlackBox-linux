MODULE DevCPB;	(* RC 6.3.89 / 24.5.93 / 17.2.94 / bh 16.6.94 *)
(* build parse tree *)
(* Component Pascal version, bh, 07 Feb 2002 *)
	...
	BEGIN
		...
		ELSE err(127); node := DevCPT.NewNode(Nvar);
		END ;
...
*)
	...
	PROCEDURE DivModConst (x, y: DevCPT.Const; div: BOOLEAN; VAR type: DevCPT.Struct);	(* x := x DIV y | x MOD y *)
	BEGIN
		IF type.form IN intSet THEN
	...
	PROCEDURE NewString*(str: DevCPT.String; len: INTEGER): DevCPT.Node;
		VAR x: DevCPT.Node;
	BEGIN
		x := DevCPT.NewNode(Nconst); x.conval := DevCPT.NewConst(); x.typ := DevCPT.string8typ;
		x.conval.intval := DevCPM.ConstNotAlloc; x.conval.intval2 := len; x.conval.ext := str;
		RETURN x
	END NewString;
	...
	BEGIN
		...
		ELSIF (typ.comp IN {Array, DynArr}) & (typ.BaseTyp.form = Char16) OR (typ.form = String16) THEN
			...
			ELSIF (ntyp.comp IN {Array, DynArr}) & (ntyp.BaseTyp.form = Char16) OR (ntyp.form = String16) THEN (* ok *)
			ELSE err(e)
			END
		ELSE err(e)
		END
	...
	BEGIN f := x.typ.form; g := typ.form;
		IF x.class = Nconst THEN
			...
			END;
			x.obj := NIL
		ELSIF (x.class = Nmop) & (x.subcl = conv) & (DevCPT.Includes(f, x.left.typ.form) OR DevCPT.Includes(f, g)) THEN
	...
	BEGIN
		...
		IF f IN intSet + realSet THEN
			IF g IN intSet + realSet THEN
				IF (f = Real32) & (right.class = Nconst) & (g IN realSet)
					& ((ABS(right.conval.realval) <= DevCPM.MaxReal32) OR (ABS(right.conval.realval) = DevCPM.InfReal))
				OR (g = Real32) & (left.class = Nconst) & (f IN realSet)
					& ((ABS(left.conval.realval) <= DevCPM.MaxReal32) OR (ABS(left.conval.realval) = DevCPM.InfReal)) THEN
		...
		ELSIF f IN charSet + {String8, String16} THEN
			IF g IN charSet + {String8, String16} THEN
				...
				IF (new.form IN {String8, String16})
					& ((f IN charSet) & (left.class # Nconst) OR (g IN charSet) & (right.class # Nconst)) THEN err(100) END
			ELSE err(100)
			END
	...
	BEGIN
		...
		IF (newPar.class = Nvarpar) & ODD(newPar.obj.sysflag DIV newBit) THEN
			IF (iidPar.class = Nvarpar) & ODD(iidPar.obj.sysflag DIV iidBit) & (iidPar.obj.mnolev = newPar.obj.mnolev) THEN (* ok *)
			ELSE err(168)
			END
		...
		ELSE
			...
			ELSE
				...
				| minus:
					...
					IF ~(f IN intSet) OR (y.class # Nconst) OR ~EqualConst(y.conval, zero, f) THEN NewOp(op, typ, z, y) END;
				| min, max:
	...
	PROCEDURE CheckAssign* (x: DevCPT.Struct; VAR ynode: DevCPT.Node);	(* x := y, checks assignment compatibility *)
	...
	BEGIN
		...
		| Int8, Int16, Int32, Int64, Real32, Real64:	(* SR *)
			...
			END
(*			
			IF ~(g IN intSet + realSet) OR ~DevCPT.Includes(f, g) & (~(g IN intSet) OR (ynode.class # Nconst)) THEN err(113)
...
*)
		...
		| Pointer:
			...
			IF DevCPT.Extends(y, x)
				OR (g = NilTyp)
				OR (g = Pointer)
					& ((x = DevCPT.sysptrtyp) OR (DevCPM.java IN DevCPM.options) & (x = DevCPT.anyptrtyp)) THEN (* ok *)
			...
			ELSIF b.untagged & (ynode.class = Nmop) & (ynode.subcl = adr) THEN	(* p := ADR(r) *)
				...
				END
			ELSIF (b.sysflag = jstr) & ((g = String16) OR (ynode.class = Nconst) & (g IN {Char8, Char16, String8})) THEN
		...
		| Comp:
			...
			ELSIF g IN {Char8, Char16, String8, String16} THEN
				...
				END;
				IF (x # DevCPT.guidtyp) & (x.comp = Array) & (ynode.class = Nconst) & (ynode.conval.intval2 > x.n) THEN err(114) END
	...
	BEGIN
		IF ~(DevCPM.oberon IN DevCPM.options)
			& ((x.typ = DevCPT.lreal64typ) OR (x.typ = DevCPT.lint64typ) OR (x.typ = DevCPT.lchar16typ)) THEN err(198) END
	END CheckOldType;
	...
	BEGIN x := par0; f := x.typ.form;
		CASE fctno OF
		  haltfn: (*HALT*)
				IF (f IN intSet - {Int64}) & (x.class = Nconst) THEN
					...
					ELSE err(218)
					END
				ELSIF (DevCPM.java IN DevCPM.options) & ((x.class = Ntype) OR (x.class = Nvar)) & (x.typ.form = Pointer) THEN
					BindNodes(Ntrap, DevCPT.notyp, x, x)
				ELSE err(69)
				END ;
				x.typ := DevCPT.notyp
		...
		| shortfn: (*SHORT*)
				IF (x.class = Ntype) OR (x.class = Nproc) THEN err(126)
				ELSE
					IF (x.typ.comp IN {Array, DynArr}) & (x.typ.BaseTyp.form IN charSet) THEN StrDeref(x); f := x.typ.form END;
		...
		| getrfn, putrfn: (*SYSTEM.GETREG, SYSTEM.PUTREG*)
				IF (f IN intSet) & (x.class = Nconst) THEN
					IF (x.conval.intval < DevCPM.MinRegNr) OR (x.conval.intval > DevCPM.MaxRegNr) THEN err(220) END
				ELSE err(69)
				END
		| valfn: (*SYSTEM.VAL*)
	...
	PROCEDURE StPar1*(VAR par0: DevCPT.Node; x: DevCPT.Node; fctno: BYTE);	(* x: second parameter of standard proc *)
	...
	BEGIN p := par0; f := x.typ.form;
		...
		| inclfn, exclfn: (*INCL, EXCL*)
				...
				ELSIF f IN intSet THEN
					IF f = Int64 THEN Convert(x, DevCPT.int32typ) END;
					IF (x.class = Nconst) & ((0 > x.conval.intval) OR (x.conval.intval > DevCPM.MaxSet)) THEN err(202) END ;
					p := NewOp(Nassign, fctno, p, x)
				ELSE err(111)
				END ;
				p.typ := DevCPT.notyp
	...
	PROCEDURE StParN*(VAR par0: DevCPT.Node; x: DevCPT.Node; fctno, n: SHORTINT);	(* x: n+1-th param of standard proc *)
	...
	PROCEDURE DynArrParCheck (ftyp: DevCPT.Struct; VAR ap: DevCPT.Node; fvarpar: BOOLEAN);	(* check array compatibility *)
	...
	BEGIN
		...
		IF ft.form # Undef THEN
			...
			IF fp.mode = VarPar THEN
				...
				ELSIF (ft.comp = DynArr) & ~ft.untagged & (ft.n = 0) & (ap.class = Ndop) & (ap.subcl = thisarrfn) THEN (* ok *)
				ELSE
					IF fp.vis = inPar THEN
						IF (ft = DevCPT.guidtyp) & (ap.class = Nconst) & (at.form = String8) THEN StringToGuid(ap); at := ap.typ
(*
...
*)
					...
					ELSE
						...
						ELSIF (ap.obj # NIL) & ODD(ap.obj.sysflag DIV newBit) & ~ODD(fp.sysflag DIV newBit) THEN err(167)
		...
		ELSE
			...
			LOOP
				IF left.class = Nvarpar THEN
					WHILE (right.class = Nindex) OR (right.class = Nfield) OR (right.class = Nguard) DO right := right.left END;
	...
	PROCEDURE CheckBuffering* (VAR n: DevCPT.Node; left: DevCPT.Node; par: DevCPT.Object; VAR stat, last: DevCPT.Node);
	...
	BEGIN
		IF n # NIL THEN
			...
			IF useTable IN opts THEN
				...
				IF n.conval.intval2 > n.conval.intval THEN DevCPM.LogW("-"); DevCPM.LogWNum(n.conval.intval2, 1) END;
