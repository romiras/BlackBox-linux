MODULE DevCPT;	(* NW, RC 6.3.89 / 24.2.94 / bh 7.9.94 *)
...
*)	
	...
	BEGIN
		...
		IF (x.comp = Record) & (y.comp = Record) THEN
			IF y = anytyp THEN RETURN TRUE END;
...
*)
	...
	BEGIN
		IF ~obj.fpdone THEN
			...
			IF obj.mode IN {LProc, XProc, CProc, Var, Typ} THEN
...
(*-------------------------- Import --------------------------*)
	...
	BEGIN
		...
		| Int64:
			...
			WHILE (y < 268435456 (*2^28*)) & (ch >= 80X) DO x := x + (ORD(ch) - 128) * y; y := y * 128; DevCPM.SymRCh(ch) END;
		...
		IF obj # NIL THEN
			...
			IF obj.mode IN {Con, Typ, Var, LProc, XProc, CProc, IProc} THEN
				...
				ELSIF obj.vis # internal THEN
					...
					IF obj.mode IN {LProc, XProc, CProc, Var} THEN
