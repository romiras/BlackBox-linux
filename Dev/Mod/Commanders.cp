...
(* OmInc, cas 18 Jul 2000 *)

	(* bj	18.07.00	added the type EndView *)
	(* dg	03.03.99	changed Kernel.ThisMod => Kernel.ThisLoadedMod *)
	(* dg 09.12.98	unload also works for quoted commands *)
	(* dg 24.10.98	ASSERT added which prevents cleander from beeing installed twice *)
	(* dg 28.09.98	trap cleaner added for global par *)
	(* dg 24.09.98	made commanders wrapper clean *)
	(* dg 24.09.98	clean up: type field, i.e. begin and end commanders elinimated *)
	(* bh 25.01.96	refCursor in HandleCtrlMsg *)
	...
	BEGIN
		...
		font.GetBounds(asc, dsc, fw);
		size := asc + dsc;
		points[0].x := l; points[0].y := size; points[1].x := u + (size DIV 2); points[1].y := size DIV 2; points[2].x := u + (size DIV 2); points[2].y := size;     
