MODULE DevCPM;
	...
	(* cp	25.01.00	corrected ValidGuid *)
	(* bh	09.09.99	enlarged MaxHaltNr to 128 *)
	(* dg	06.05.99	replaced SetDirty-hack by transparent operations *)
	(* bh 29.11.95 sysflag handling *)
	(* mf 4.2.93 / cp 22 Aug 93 / bh 7.9.94 *)

(* Component Pascal version, bh, 01 Nov 1994 *)
	...
	CONST
		...
		MaxStruct* = 4096;	(* must be < MAX(INTEGER) DIV 2 in object model *)
...
TYPE
	...
	BEGIN
		...
		ELSIF (sys386 IN options) & (num = -10) & (flag = 0) THEN flag := -10
	...
	BEGIN
		...
		IF (options * {sys386, sys68k, interface, com} # {}) THEN
			...
			ELSIF ((num = 8) OR (id = "new")) & (com IN options) THEN
...
*)
	...
	BEGIN
		fp := SYSTEM.ROT(ORD(BITS(fp) / BITS(val)), 1)
	END FPrint;
