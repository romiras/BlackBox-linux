MODULE DevAnalyzer;	(* NW, RC 6.3.89 / 10.2.94 / object model 4.12.93 / bh 7.9.94 *)
...
*)	END DumpObj;
	...
	PROCEDURE CheckOuter;	(*<< check outer scopes for obj with OPS.name*)
		VAR o: CPT.Object;
	BEGIN IF options.intermediate THEN CPT.Find(CPS.name, o); IF o # NIL THEN DumpObj(o); err2(outerScope, CPM.curpos) END END
	END CheckOuter;
	...
	BEGIN
		...
		IF (sym = ident) OR (sym = var) OR (sym = in) OR (sym = out) THEN
			LOOP
				...
				LOOP
					IF sym = ident THEN
						...
						IF mode = VarPar THEN	(*<< parameters are set, except for out parameters *)
							...
							ELSIF vis = outPar THEN par.num := clean	(* out parameters are not initialized, except for pointers and procedure variables *)
				...
				IF (mode = VarPar) & (vis = inPar) & (typ.form # Undef) & (typ.form # Comp) & (typ.sysflag = 0) THEN err(177) END;
...
*)
			...
			IF sym # rparen THEN
				LOOP
					IF n = 0 THEN
						...
						| lenfn:
							qualident(id); x := CPB.NewLeaf(id); selector(x);
							Selector(x, SelNoAssign, SelNoVarPar, SelNoUseBSet, CPM.errpos)	(* no usedBSet message if LEN(id) *)
						ELSE Expression(x)
						END;
						CPB.StPar0(x, m); n := 1
					ELSIF n = 1 THEN
	...
	BEGIN
		...
		IF sym = ident THEN
			...
			selector(x);
			IF (x.class = Nfield) & (x.obj.mode = TProc) & (x.obj.link.mode = VarPar) & (x.obj.link.typ.comp = Record) THEN	(*<< receiver is var parameter and record *)
			...
			IF (x.class = Nproc) & (x.obj.mode = SProc) THEN
				IF ~((x.obj.adr IN {ordfn, minfn, maxfn, chrfn, sizefn, adrfn, valfn}) OR (x.obj.adr = bitsfn)) THEN INC(nofStats) END;	(*<<*)
...
*)
		...
		ELSIF sym = string THEN
			x := CPB.NewString(CPS.str, CPS.intval);
			CPS.Get(sym)
		ELSIF sym = nil THEN
	...
	PROCEDURE ProcedureDeclaration(VAR x: CPT.Node);
		...
		BEGIN
			...
			IF sym = ident THEN
				...
				proc^.link.adr := pos1;	(*<< *)
				(*
				IF objMode = VarPar THEN proc^.link.num := setUsed ELSE proc^.link.num := set END;	(*<< receiver is set/setUsed *)
				*)
...
*)
	PROCEDURE StatSeq(VAR stat: CPT.Node);
		VAR fpar, id, t: CPT.Object; idtyp: CPT.Struct; e: BOOLEAN; oldSym: BYTE;	(*<< oldSym*)
				s, x, y, z, apar, last, lastif, pre, lastp: CPT.Node; pos, p: INTEGER; pos1, lastSemiPos: INTEGER; 	(*<< pos1, pos2, oldPos *)
				lastSemi: BOOLEAN;	(*<<*)
			...
			IF sym = import THEN CPS.Get(sym);
				...
				LOOP
					IF sym = ident THEN
						...
						CPS.name := aliasName$;	(*<<*)
						CPT.Find(CPS.name, obj); IF obj # NIL THEN obj.adr := CPM.errpos; obj.num := set END (*<< modules are set by default *)
					ELSE err(ident)
					END ;
					IF sym = comma THEN CPS.Get(sym)
					ELSIF sym = ident THEN err(comma)
...
(*<<OP2 procedures *)
	...
	*)
	...
	PROCEDURE LoadOptions*;
		VAR res: INTEGER;
	BEGIN
		HostRegistry.ReadBool(regKey + 'varPar', options.varpar, res); IF res # 0 THEN options.varpar := FALSE END;
		HostRegistry.ReadBool(regKey + 'exported', options.exported, res); IF res # 0 THEN options.exported := FALSE END;
		HostRegistry.ReadBool(regKey + 'intermediate', options.intermediate,res); IF res # 0 THEN options.intermediate := FALSE END;
		HostRegistry.ReadBool(regKey + 'levels', options.levels, res); IF res # 0 THEN options.levels := FALSE END;
		HostRegistry.ReadBool(regKey + 'tbProcs', options.tbProcs, res); IF res # 0 THEN options.tbProcs := FALSE END;
		HostRegistry.ReadBool(regKey + 'semicolons', options.semicolons, res); IF res # 0 THEN options.semicolons := FALSE END;
		HostRegistry.ReadBool(regKey + 'sideEffects', options.sideEffects, res); IF res # 0 THEN options.sideEffects := FALSE END;
		Dialog.Update(options)
	END LoadOptions;
