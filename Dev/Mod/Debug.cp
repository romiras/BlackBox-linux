...
(* cp, bh 20 Feb 2001 *)
	...
	PROCEDURE^ ShowVar (ad, ind: INTEGER; f, c: SHORTCHAR; desc: Kernel.Type; ptr: ArrayPtr; back: RefView; VAR name, sel: Name);
	...
	PROCEDURE ShowArray (a, ind: INTEGER; desc: Kernel.Type; ptr: ArrayPtr; back: RefView; VAR sel: Name);
		VAR f: SHORTCHAR; i, n, m, size, len: INTEGER; name: Kernel.Name; eltyp, t: Kernel.Type; vi: SHORTINT; vs: BYTE; str: Dialog.String;
	BEGIN
	...
	PROCEDURE UnloadModuleList*;
		VAR c: TextControllers.Controller; s: TextMappers.Scanner; res, beg, end: INTEGER; ok, num: BOOLEAN; linked: ARRAY 16 OF CHAR;
	BEGIN
		c := TextControllers.Focus();
		IF (c # NIL) & c.HasSelection() THEN
			c.GetSelection(beg, end);
	...
	BEGIN
		...
		ELSE
			...
			IF Kernel.err = 127 THEN out.WriteSString("  (abstract method called)")
			ELSIF Kernel.err = 126 THEN out.WriteSString("  (not yet implemented)")
