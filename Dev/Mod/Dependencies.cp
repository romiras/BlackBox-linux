MODULE DevDependencies;
	...
	PROCEDURE GetImplicitDependencies(IN name: String; OUT imps: POINTER TO ARRAY OF String; OUT nofImps: INTEGER);
	...
	BEGIN
		RETURN l.subsystem.isExpanded & ~l.isHidden & ~l.subsystem.isHidden & (~l.subsystem.isBasic OR showBasic)
	END Visible;
	...
	BEGIN
		...
		ELSE
			...
			IF found THEN
				...
				IF sl # NIL THEN sslist.isExpanded := TRUE; sslist.level := 1; sslist.isStart := TRUE; tmpList.isStart := TRUE END;
	...
	BEGIN
		...
		WHILE s # NIL DO
			IF s.Visible(v.showBasic) & OverLap(s.l, s.b - s.h, s.l + s.w, s.b, x1, y1, x2, y2)  THEN 
				...
				END
			ELSE
				IF ~(Controllers.extend IN modifiers) & ~(Controllers.modify IN modifiers) THEN s.isSelected := FALSE END
			END;
			s := s.next
		END
	END Select;
	...
	BEGIN
		...
		WHILE tmpList # NIL DO tmpList.w := v.selfont.StringWidth(tmpList.name); tmpList.h := h; tmpList := tmpList.next END;
		ssList := v.subsystems;
		WHILE ssList # NIL DO ssList.w := v.selfont.StringWidth("[" + ssList.name + "]"); ssList.h := h; ssList := ssList.next END
	END SetExtents;
	...
	BEGIN
		WITH source: SubsystemList DO
			s.name := source.name; s.isExpanded := source.isExpanded; 
			s.isLayedout := source.isLayedout; s.isHidden := source.isHidden; s.isBasic := source.isBasic; s.isStart := source.isStart;
	...
	BEGIN
		...
		IF v # NIL THEN 
			...
			IF v.showBasic THEN Views.Do(v, "Hide Basic Systems", hbop) ELSE Views.Do(v, "Show Basic Systems", hbop) END
		END
		...
		BEGIN
			IF full OR node.subsystem.isExpanded THEN
				...
				IF ml = NIL THEN
					...
					END;
					ml := clist; prev := ml;
					WHILE (ml # NIL) & (ml.list # node)  DO prev := ml; ml := ml.next END; (* could have been implicitly imported *)
					IF ml = NIL THEN
						NEW(ml); ml.list := node;
	...
	BEGIN
		...
		WHILE nonImplList # NIL DO
			...
			ELSE
				WHILE nonImplList.list.name$ # fullList.list.name$ DO fullList.list.onlyImplicit := TRUE; fullList := fullList.next END
			END;
	...
	BEGIN
		...
		IF startList # NIL THEN StartAnalysis(startList, NIL, implicit, v) ELSE Dialog.ShowMsg("#Dev:NoModuleNameSelected") END
	END Analyze;
