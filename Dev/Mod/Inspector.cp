MODULE DevInspector;
	...
	IMPORT Services, Stores, Views, Controllers, Properties, Containers, Dialog, Controls;
	...
	PROCEDURE GetTypeName (v: Views.View; VAR t, s: ARRAY OF CHAR);
		VAR i, j: INTEGER; ch: CHAR; c: Containers.Controller; w: Views.View;
	BEGIN
		s := ""; t := "";
		IF multiView THEN
			c := Containers.Focus();
			IF c # NIL THEN
				c.GetFirstView(TRUE, v); w := v;
				WHILE w # NIL DO
					IF ~Services.SameType(w, v) THEN RETURN END;
					c.GetNextView(TRUE, w)
				END
			END
		END;
		IF v # NIL THEN
			...
			s := "#Dev:"; j := 5;
			i := 0; ch := t[0]; WHILE ch # 0X DO s[j] := ch; INC(j); INC(i); ch := t[i] END; s[j] := 0X;
			Dialog.MapString(s, s)
		END
	END GetTypeName;

	PROCEDURE PollProp (v: Views.View);
		VAR msg: Properties.PollMsg; q: Properties.Property; p: Controls.Prop; oldp: BOOLEAN;
	BEGIN
