MODULE DevPacker;
	...
	IMPORT 
		Kernel,  StdLog, Files, Stores,
		DevCommanders, TextMappers, TextModels, TextViews, Views, HostPackedFiles, Strings, Dialog;
	...
	BEGIN
		...
		WHILE l # NIL DO
			...
			IF f # NIL THEN
				l.len := f.Length(); r := f.NewReader(NIL); r.SetPos(0);
				l.adr := exe.Length(); wr.SetPos(l.adr);
		...
		WHILE l # NIL DO
			...
			l := l.next
		END;
		sw.WriteInt(HostPackedFiles.magicnum); sw.WriteInt(tableadr);
		IF err > 0 THEN 
	...
	BEGIN
		...
		WHILE l # NIL DO
			...
			ELSIF Files.dir.SameFile(exeloc, exefile, loc, fi.name) THEN
				StdLog.String('Cannot pack a file into itself. (' + l.name + ' cannot be both the target exe and included in the list)');
	...
	PROCEDURE FileName (rd: TextModels.Reader; end: INTEGER; OUT name: ARRAY OF CHAR; OUT ok: BOOLEAN);
	...
	PROCEDURE ParseExe (rd: TextModels.Reader; end: INTEGER; OUT exepath, exefile: Files.Name; OUT ok: BOOLEAN);
		VAR name: Files.Name;
	BEGIN
		ok := FALSE;
		GetCh(rd);
...
 DevPacker.PackThis exefile.exe := Host/Mod/files.odc => Host/Mod/Hoi.odc "host\Mod\CFrames.odc" Std/Mod/Log.odc => 
'Std/Mod/Logg.odc'
