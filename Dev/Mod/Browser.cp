...
*)
	...
	PROCEDURE ReadHeader (file: Files.File; VAR view: Views.View; VAR title: Views.Title);
		VAR ch, ch1: SHORTCHAR; n, i, p, fp, hs, ms, ds, cs, vs, m: INTEGER; name: Kernel.Name; first: BOOLEAN;
	...
	PROCEDURE Init*;
		VAR res: INTEGER;
	BEGIN
		HostRegistry.ReadBool(regKey + 'hints', options.hints, res); IF res # 0 THEN options.hints := FALSE END;
		HostRegistry.ReadBool(regKey + 'flatten', options.flatten, res); IF res # 0 THEN options.flatten := TRUE END;
		HostRegistry.ReadBool(regKey + 'formatted', options.formatted, res); IF res # 0 THEN options.formatted := FALSE END;
