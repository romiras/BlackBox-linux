...
*)
	...
	BEGIN
		...
		| Int64:
			...
			WHILE (c # NIL) & ((con.setval # c.setval) OR (con.intval # c.intval2) OR (con.realval # c.realval)) DO c := c.link END;
	...
	PROCEDURE OutDesc (desc: DevCPT.Struct);
		VAR m: DevCPT.Object; i, nofptr, flddir, size: INTEGER; t, xb: DevCPT.Struct; form, lev, attr: BYTE; name: DevCPT.Name;
	BEGIN
		...
		ELSE
			...
			IF desc.form # ProcTyp THEN OutStruct(desc.BaseTyp, TRUE) END
		END
	END OutDesc;
		...
		IF obj # NIL THEN
			...
			ELSIF obj.used THEN
				...
				IF obj.mode = Var THEN
					DevCPM.ObjW(SHORT(CHR(mVar))); OutName(obj.name^); DevCPM.ObjWNum(obj.fprint); OutLink(obj.links)
				ELSIF obj.mode IN {XProc, IProc} THEN
					DevCPM.ObjW(SHORT(CHR(mProc))); OutName(obj.name^); DevCPM.ObjWNum(obj.fprint); OutLink(obj.links)
				ELSE ASSERT(obj.mode IN {Con, CProc});
	...
	BEGIN
		...
		WHILE m # NIL DO
			obj := m.nlink;
			WHILE obj # NIL DO
				IF obj.mode = Var THEN DevCPM.ObjW(SHORT(CHR(mVar))) ELSE DevCPM.ObjW(SHORT(CHR(mProc))) END;
