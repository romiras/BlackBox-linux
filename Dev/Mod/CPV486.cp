MODULE DevCPV486;	(* bh 3.12.93 / 7.9.94 *)
	...
	CONST
		...
		addRefFP = 48H;	(* fingerprint of AddRef and Release procedures *)
		intHandlerFP = 12A8H;	(* fingerprint of InterfaceTrapHandler *)
		numPreIntProc = 2;
...
*)
	...
	BEGIN
		...
		IF proc # NIL THEN
			...
			ELSIF (proc.mode = TProc) & (proc.num >= numPreIntProc) & (proc.num < numPreIntProc + NumOfIntProc(proc.link.typ)) THEN
				INCL(proc.conval.setval, isCallback);
	...
	PROCEDURE Variables(var: DevCPT.Object; VAR varSize: INTEGER); (* allocates only offsets, regs allocated in DevCPC486.Enter *)
	...
	BEGIN
		...
		IF useLen & (left.class = Nderef) & (left.typ.comp = DynArr) & ~left.typ.untagged THEN	(* keep len descriptor *)
	...
	BEGIN
		obj := proc.link;
		WHILE (obj # NIL) & ((obj.mode # Var) OR ~obj.used OR ~DevCPC486.ContainsIPtrs(obj.typ)) DO obj := obj.link END;
		IF obj = NIL THEN
	...
	PROCEDURE CompareIntTypes (typ: DevCPT.Struct; VAR id: DevCPL486.Item; VAR exit: DevCPL486.Label; VAR num: INTEGER);
	...
	PROCEDURE expr(n: DevCPT.Node; VAR x: DevCPL486.Item; hint, stop: SET);
		VAR y, z: DevCPL486.Item; f, g: BYTE; cval: DevCPT.Const; false, true: DevCPL486.Label; uy: SET; sy: INTEGER; r: REAL;
	BEGIN
		f := n.typ.form;
	...
	BEGIN
		IF (left.class = Nderef) & (left.typ.comp = DynArr) & ~left.typ.untagged THEN
			expr(left, x, wreg - {CX}, {DI})	(* keep len descriptor *)
	...
	BEGIN
		expr(n.left, x, {}, {mem, con, short}); else := DevCPL486.NewLbl;
