MODULE DevTypeLibs;
	...
	BEGIN
		...
		ELSIF ODD(v.vt DIV WinOle.VT_BYREF) THEN
			...
			| WinOle.VT_I4: IF v.u.plVal[0] = 80000000H THEN out.WriteString("80000000H")
								ELSE out.WriteInt(v.u.plVal[0])
								END
			| WinOle.VT_R4: out.WriteReal(v.u.pfltVal[0])
			| WinOle.VT_R8: out.WriteReal(v.u.pdblVal[0])
			| WinOle.VT_BOOL: IF v.u.pboolVal[0] = 0 THEN out.WriteSString("FALSE") ELSE out.WriteSString("TRUE") END
		...
		ELSE
			...
			| WinOle.VT_I4: IF v.u.lVal = 80000000H THEN out.WriteString("80000000H")
								ELSE out.WriteInt(v.u.lVal)
								END
			| WinOle.VT_R4: out.WriteReal(v.u.fltVal)
			| WinOle.VT_R8: out.WriteReal(v.u.dblVal)
			| WinOle.VT_BOOL: IF v.u.boolVal = 0 THEN out.WriteSString("FALSE") ELSE out.WriteSString("TRUE") END
	...
	PROCEDURE WriteTypeName (tinfo: WinOleAut.ITypeInfo; convert, isThis: BOOLEAN; VAR out: TextMappers.Formatter);
	...
	PROCEDURE WriteHandleName (t: WinOleAut.HREFTYPE; tinfo: WinOleAut.ITypeInfo; opts: SET; VAR out: TextMappers.Formatter);
	...
	PROCEDURE WriteType (VAR t: WinOleAut.TYPEDESC; tinfo: WinOleAut.ITypeInfo; opts: SET; VAR out: TextMappers.Formatter);
	...
	BEGIN
		...
		| WinOle.VT_CY: IF wrapper IN opts THEN out.WriteSString("CtlT.OleCy") ELSE out.WriteSString("CURRENCY") END
		| WinOle.VT_DATE: IF wrapper IN opts THEN out.WriteSString("CtlT.OleDate") ELSE out.WriteSString("DATE") END
		| WinOle.VT_BSTR: IF wrapper IN opts THEN out.WriteSString("CtlT.Strg") ELSE out.WriteSString("WinOle.BSTR") END
		| WinOle.VT_DISPATCH:
		...
		| WinOle.VT_BOOL: IF wrapper IN opts THEN out.WriteSString("BOOLEAN") ELSE out.WriteSString("WinOle.VARIANT_BOOL") END
		| WinOle.VT_VARIANT: IF wrapper IN opts THEN out.WriteSString("CtlT.Any") ELSE out.WriteSString("WinOleAut.VARIANT") END
		...
		| WinOle.VT_VOID: IF browse IN opts THEN out.WriteSString("VOID") ELSE out.WriteSString("RECORD END") END
		...
		| WinOle.VT_SAFEARRAY:
			...
			END
		| WinOle.VT_CARRAY: out.WriteSString("ARRAY "); i := 0;
			WHILE i < t.u.lpadesc.cDims DO out.WriteInt(t.u.lpadesc.rgbounds[i].cElements); out.WriteChar(" "); INC(i) END;
		...
		| WinOle.VT_LPSTR: IF wrapper IN opts THEN out.WriteSString("CtlT.Strg") ELSE out.WriteSString("PtrSTR") END
		| WinOle.VT_LPWSTR: IF wrapper IN opts THEN out.WriteSString("CtlT.Strg") ELSE out.WriteSString("PtrWSTR") END
	...
	BEGIN
		...
		ELSE
			...
			IF var.varkind = WinOleAut.VAR_STATIC THEN
				...
				IF res >= 0 THEN
					out.WriteSString(" [");
					IF s # NIL THEN out.WriteChar('"'); WriteBSTR(s, out); out.WriteSString('", '); WinOleAut.SysFreeString(s) END;
		...
		IF (s # NIL) OR (browse IN opts) THEN
			...
			IF browse IN opts THEN
				...
				IF ODD(var.wVarFlags DIV WinOleAut.VARFLAG_FREADONLY) THEN out.WriteSString(", readonly") END;
				IF ODD(var.wVarFlags DIV WinOleAut.VARFLAG_FSOURCE) THEN out.WriteSString(", source") END;
				IF ODD(var.wVarFlags DIV WinOleAut.VARFLAG_FBINDABLE) THEN out.WriteSString(", bindable") END;
				IF ODD(var.wVarFlags DIV WinOleAut.VARFLAG_FDISPLAYBIND) THEN out.WriteSString(", display") END;
				IF ODD(var.wVarFlags DIV WinOleAut.VARFLAG_FDEFAULTBIND) THEN out.WriteSString(", default") END;
...
*)
	...
	BEGIN
		...
		ELSE
			...
			ELSIF ODD(invoke DIV WinOleAut.INVOKE_PROPERTYGET) THEN out.WriteSString("CtlC.CallGetMethod(this, ")
			ELSIF ODD(invoke DIV WinOleAut.INVOKE_PROPERTYPUT) THEN out.WriteSString("CtlC.CallPutMethod(this, ")
			ELSIF ODD(invoke DIV WinOleAut.INVOKE_PROPERTYPUTREF) THEN out.WriteSString("CtlC.CallPutRefMethod(this, ")
				...
				IF WinOleAut.IMPLTYPEFLAG_FSOURCE * flags = {} THEN
					...
					IF iattr.typekind = WinOleAut.TKIND_DISPATCH THEN
						...
						IF si # NIL THEN
							...
							IF WinOleAut.IMPLTYPEFLAG_FDEFAULT * flags = {}  THEN out.WriteChar("_"); WriteBSTR(si, out) END;
...
*)
			...
			IF (s # NIL) OR (browse IN opts) THEN
				...
				IF browse IN opts THEN
					...
					ELSIF func.invkind = WinOleAut.INVOKE_PROPERTYPUTREF THEN out.WriteSString(", putref")
					END;
					IF ODD(func.wFuncFlags DIV WinOleAut.FUNCFLAG_FRESTRICTED) THEN out.WriteSString(", restricted") END;
					IF ODD(func.wFuncFlags DIV WinOleAut.FUNCFLAG_FSOURCE) THEN out.WriteSString(", source") END;
					IF ODD(func.wFuncFlags DIV WinOleAut.FUNCFLAG_FBINDABLE) THEN out.WriteSString(", bindable") END;
					IF ODD(func.wFuncFlags DIV WinOleAut.FUNCFLAG_FREQUESTEDIT) THEN out.WriteSString(", request") END;
					IF ODD(func.wFuncFlags DIV WinOleAut.FUNCFLAG_FDISPLAYBIND) THEN out.WriteSString(", display") END;
					IF ODD(func.wFuncFlags DIV WinOleAut.FUNCFLAG_FDEFAULTBIND) THEN out.WriteSString(", default") END;
					IF ODD(func.wFuncFlags DIV WinOleAut.FUNCFLAG_FHIDDEN) THEN out.WriteSString(", hidden") END;
					IF (func.funckind = WinOleAut.FUNC_PUREVIRTUAL) OR (func.funckind = WinOleAut.FUNC_VIRTUAL) THEN
	...
	BEGIN
		...
		ELSE
			...
			ELSE
				...
				ELSE
					out.WriteSString("RECORD ");
					IF (attr.typekind = WinOleAut.TKIND_INTERFACE) OR (attr.typekind = WinOleAut.TKIND_DISPATCH) THEN
				...
				IF func.invkind = WinOleAut.INVOKE_PROPERTYGET THEN
					...
					WHILE j < attr.cFuncs DO
						...
						IF (pfunc.memid = func.memid)
							...
							IF ODD(pfunc.invkind DIV WinOleAut.INVOKE_PROPERTYPUT) THEN out.WriteInt(pfunc.cParams)
	...
	BEGIN
		...
		WHILE i < attr.cImplTypes DO
			...
			IF WinOleAut.IMPLTYPEFLAG_FDEFAULTVTABLE * flags # {} THEN out.WriteSString("defaultVtable ") END;
	...
	PROCEDURE ShowLibrary (tlib: WinOleAut.ITypeLib; name: ARRAY OF CHAR; opts: SET; VAR out: TextMappers.Formatter);
		VAR s1, s2, s3: WinOle.BSTR; str: ARRAY 256 OF SHORTCHAR; i, n, impPos: INTEGER; res: COM.RESULT;
			attr: WinOleAut.PtrTLIBATTR; tinfo, ti: WinOleAut.ITypeInfo; kind: WinOleAut.TYPEKIND; t: WinOleAut.HREFTYPE;
	BEGIN
		...
		END;
		out.WriteSString(", version: "); out.WriteInt(attr.wMajorVerNum); out.WriteChar("."); out.WriteInt(attr.wMinorVerNum);
		...
		WHILE i < n DO
			...
			IF (kind = WinOleAut.TKIND_ALIAS) OR (kind = WinOleAut.TKIND_ENUM)
					...
					OR ~(interface IN opts) & (kind IN {WinOleAut.TKIND_DISPATCH, WinOleAut.TKIND_COCLASS}) THEN
	...
	PROCEDURE AutomationInterface* (fileName: ARRAY 256 OF CHAR; modName: ARRAY 64 OF CHAR): TextModels.Model;
	...
	PROCEDURE CustomInterface* (fileName: ARRAY 256 OF CHAR; modName: ARRAY 64 OF CHAR): TextModels.Model;
	(* not yet tested *)
		VAR t: TextModels.Model; out: TextMappers.Formatter; res: COM.RESULT; tlib: WinOleAut.ITypeLib;
	BEGIN
		t := TextModels.dir.New();
