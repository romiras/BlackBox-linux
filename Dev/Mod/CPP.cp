MODULE DevCPP;	(* NW, RC 6.3.89 / 10.2.94 / object model 4.12.93 / bh 7.9.94 *)
...
(* Component Pascal version, bh, 09 Mar 2001 *)
	...
	PROCEDURE FixType (struct: DevCPT.Struct; obj: DevCPT.Object; typ: DevCPT.Struct; pos: INTEGER);	(* fix forward reference *)
	...
	PROCEDURE FormalParameters(VAR firstPar: DevCPT.Object; VAR resTyp: DevCPT.Struct; VAR name: DevCPT.String);
			...
			ELSE
				...
				IF id.mode = Typ THEN
					IF ~(DevCPM.oberon IN DevCPM.options)
						& ((id.typ = DevCPT.lreal64typ) OR (id.typ = DevCPT.lint64typ) OR (id.typ = DevCPT.lchar16typ)) THEN err(198) END;
					typ := id.typ
		...
		IF sym # rparen THEN
			...
			LOOP Expression(apar);
				IF fpar # NIL THEN
					...
					IF anchorVarPar & (fpar.mode = VarPar) & (apar.class > Nvarpar) & ~(DevCPM.java IN DevCPM.options) THEN
						n := apar;
							...
							IF (obj # NIL) & (obj.mode = TProc) THEN
								IF sym = arrow THEN  (* super call *) DevCPS.Get(sym);
									...
									IF y.obj # NIL THEN
										...
										IF (proc.link = NIL) OR (proc.link.link # y.obj) (* OR (proc.link.name^ # name) *) THEN err(75) END ;
										typ := y.obj.typ;
										IF typ.form = Pointer THEN typ := typ.BaseTyp END ;
...
*)
		...
		ELSIF sym = string THEN
			x := DevCPB.NewString(DevCPS.str, DevCPS.intval);
			DevCPS.Get(sym)
		ELSIF sym = nil THEN
